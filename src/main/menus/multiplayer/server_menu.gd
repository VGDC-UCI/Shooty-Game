...
func _display_pregame_screen() -> void:
	...
	get_node( "PregameScreen/Lobby" ).add_new_player( gamestate.get_player_config()["name"], false)
	# get_node( "PregameScreen/PlayerList" ).add_item( gamestate.get_player_name() + " (You)" )
	gamestate.player_config = get_node( "PregameScreen/Lobby" ).get_player_configs()[0]
...
func refresh_player_list() -> void:
	...
	# player_list.sort()
	get_node("PregameScreen/Lobby").clear()

	get_node( "PregameScreen/Lobby" ).add_existing_player(gamestate.get_player_config(), false)

	# Every player name besides the player name for this user is in this list.
	for player_config in player_list:
		get_node("PregameScreen/Lobby").add_existing_player(player_config, false, false)


func config_changed():
	gamestate.sync_player_config_for_others(get_node("PregameScreen/Lobby").get_player_configs()[0])
