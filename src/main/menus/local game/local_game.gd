...
# 		var player: Player = characters.get_class_scene(config["class_id"]).instance()
# 		level.get_node('Players').add_child(player)

# 		player.set_player_name(config["name"])
# 		player.get_node("PlayerController").control_scheme = controls.get_scheme_data(config["input_id"])
# 		player.get_node("PlayerController").using_controller = controls.get_scheme_type(config["input_id"]) == controls.types.CONTROLLER
# 		player.get_node("PlayerController").training_mode = controls.get_scheme_type(config["input_id"]) == controls.types.DUMMY
